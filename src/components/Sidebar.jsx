  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div
          className={`p-4 border-b border-neutral-900 flex items-center ${collapsed ? "justify-center" : "gap-3"}`}
        >
          {collapsed ? (
            <button
              onClick={() => setCollapsed(!collapsed)}
              className="hidden lg:flex w-12 h-12 flex-shrink-0 items-center justify-center text-neutral-600 hover:text-neutral-400 cursor-pointer text-lg rounded-lg hover:bg-neutral-900/60 transition-colors"
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                onClick={() => setMobileOpen(false)}
                className={({ isActive }) =>
                  `w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border text-xs transition-all duration-150 text-left ${
              ...
              >
                ...
                <span
                  className={`whitespace-nowrap font-medium ${collapsed ? "lg:hidden" : "block"}`}
                >
                  {item.label}
                </span>
              </NavLink>
        ...
        <div className="p-3 border-t border-neutral-900">
          <div
            className={`flex items-center justify-between gap-2 ${collapsed ? "lg:hidden" : "flex"}`}
          >
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              <div className="overflow-hidden">
                <div className="text-neutral-300 text-xs font-semibold truncate">
                  Admin
                </div>
                <div className="text-neutral-650 text-[10px] truncate">
                  {userEmail}
                </div>
              </div>
            </div>
