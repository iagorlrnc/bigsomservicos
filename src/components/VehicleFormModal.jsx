...
export default function VehicleFormModal({ onClose, onSave }) {
  const [form, setForm] = useState({ plate: "", brand: "", model: "", year: "", color: "", owner_name: "", owner_phone: "" });
  ...
  const ok = form.plate && form.brand && form.model && form.year && form.color && form.owner_name;
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-[#0d0d0d] border border-neutral-900 rounded-2xl p-5 sm:p-7 w-full max-w-[520px] max-h-[calc(100vh-3rem)] overflow-y-auto animate-scale-up">
        <div className="flex justify-between items-center mb-5.5">
          <h2 className="text-white text-lg font-semibold">Cadastrar Veículo</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Marca" required>
            <Input value={form.brand} onChange={e => setForm(f => ({ ...f, brand: e.target.value }))} placeholder="Toyota" />
          </FormField>
          <FormField label="Modelo" required>
            <Input value={form.model} onChange={e => setForm(f => ({ ...f, model: e.target.value }))} placeholder="Corolla" />
          </FormField>
          <FormField label="Ano" required>
            <Input value={form.year} onChange={e => setForm(f => ({ ...f, year: e.target.value }))} placeholder="2021" type="number" />
          </FormField>
          <FormField label="Cor" required>
            <Input value={form.color} onChange={e => setForm(f => ({ ...f, color: e.target.value }))} placeholder="Prata" />
          </FormField>
        </div>

        <div className="border-t border-neutral-900 pt-4 mt-2">
          <p className="text-[11px] text-neutral-500 font-semibold tracking-wider uppercase mb-3">DADOS DO PROPRIETÁRIO</p>
          <FormField label="Nome do Proprietário" required>
            <Input value={form.owner_name} onChange={e => setForm(f => ({ ...f, owner_name: e.target.value }))} placeholder="João da Silva" />
          </FormField>
          <FormField label="Telefone">
            <Input value={form.owner_phone} onChange={e => setForm(f => ({ ...f, owner_phone: e.target.value }))} placeholder="(63) 99999-0000" />
          </FormField>
        </div>
