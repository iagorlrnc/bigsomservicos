...
export default function CollaboratorPage({ vehicles, services, updateService, addToast }) {
  ...
  const filteredVehicles = vehiclesWithServices.filter(v =>
  ...
  const handleServiceToggle = async (svc) => {
    ...
    try {
      ...
      addToast(
        isFinished ? "Serviço retornado para execução" : "Serviço concluído com sucesso! ✓",
  ...
  const activeVehicle = plate 
    ? vehicles.find(v => v.plate.replace("-", "").toUpperCase() === plate.replace("-", "").toUpperCase())
    : null;
  const activeServices = activeVehicle ? services.filter(s => s.vehicle_id === activeVehicle.id) : [];
    ...
    <div className="min-h-screen bg-[#050505] text-[#ccc] font-sans flex flex-col">
      ...
      <header className="bg-[#080808] border-b border-neutral-900 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center font-black text-xs text-black">
            BS
          </div>
          <div>
            <h1 className="text-white font-bold text-sm tracking-wide">BIGSOM</h1>
            <p className="text-[#34d980] text-[10px] uppercase font-semibold tracking-wider">Painel do Colaborador</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-4 sm:p-5 max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
        ...
        <section className={`space-y-4 ${activeVehicle ? 'hidden md:block' : 'block'}`}>
          <div>
            <h2 className="text-white font-semibold text-sm mb-1">Veículos em Manutenção</h2>
            <p className="text-neutral-500 text-xs">Selecione um veículo para ver e marcar os serviços.</p>
          </div>
          ...
          <div className="space-y-2.5 max-h-[70vh] overflow-y-auto pr-1">
            {filteredVehicles.map(v => {
              const vServices = services.filter(s => s.vehicle_id === v.id);
              const doneCount = vServices.filter(s => s.status === "finalizado").length;
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-neutral-500 text-[11px] mt-1.5">
                        Proprietário: <span className="text-neutral-400 font-medium">{v.owner_name}</span>
                      </p>
                    </div>

                    <div className="text-right">
                      <div className={`text-xs font-bold ${
                        doneCount === totalCount ? "text-[#34d980]" : "text-amber-500"
                      }`}>
                        {doneCount}/{totalCount}
                      </div>
                      <span className="text-[9px] text-neutral-600 uppercase font-bold tracking-wider">Serviços</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <section className={`bg-[#0d0d0d] border border-neutral-900 rounded-2xl p-5 sticky top-24 space-y-4 ${activeVehicle ? 'block' : 'hidden md:block'}`}>
          {activeVehicle ? (
            <>
              <div>
                <button
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <h3 className="text-white font-bold text-base">{activeVehicle.brand} {activeVehicle.model}</h3>
                </div>
                <p className="text-neutral-500 text-xs">Marque ou desmarque os serviços realizados abaixo:</p>
              </div>
            ...
            <div className="text-center py-20 text-neutral-600 text-xs">
              Selecione um veículo ao lado para visualizar a lista de serviços a serem executados.
            </div>
          )}
        </section>
      </main>
