...
export default function ServicesPage({ vehicles, services, updateService, addToast }) {
  ...
  const handleToggle = async (svc) => {
    ...
    try {
      ...
      addToast(done ? "Serviço reaberto" : "Serviço finalizado! ✓", done ? "info" : "success");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <p className="text-neutral-500 text-xs mb-6">{services.length} serviços no total</p>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1.5 scrollbar-thin">
        {[
          ...
          const count = f.key === "all" ? services.length : services.filter(s => s.status === f.key).length;
          return (
            <button
              key={f.key}
      ...
      <div className="bg-[#0d0d0d] border border-neutral-900 rounded-xl overflow-hidden shadow-lg">
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-neutral-950/60 bg-[#0a0a0a]/55">
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider">Serviço</th>
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider">Veículo</th>
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider hidden sm:table-cell">Proprietário</th>
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider">Valor</th>
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider">Status</th>
                <th className="px-5 py-3.5 text-[10px] text-neutral-600 font-bold uppercase tracking-wider hidden md:table-cell">Data</th>
                ...
                return (
                  <tr key={s.id} className="hover:bg-neutral-900/10 transition-colors">
                    <td className="px-5 py-4">
                      <div className="text-neutral-250 font-semibold">{s.name}</div>
                      <div className="text-neutral-500 text-[11px] mt-0.5 max-w-[240px] truncate" title={s.description}>
                        {s.description}
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      {v ? (
                        ...
                        >
                          ...
                          <div className="text-neutral-650 text-[10px] tracking-wider mt-0.5 uppercase">{v.plate}</div>
                        </button>
                      ) : "-"}
                    </td>
                    <td className="px-5 py-4 text-neutral-500 font-medium hidden sm:table-cell">{v?.owner_name || "-"}</td>
                    <td className="px-5 py-4 text-amber-500 font-bold">{fmtCurrency(s.value)}</td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-md border text-[10px] ${cfg.bgClass}`}>
                        {cfg.label}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-neutral-600 font-medium hidden md:table-cell">{fmtDate(s.completed_at || s.created_at)}</td>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-neutral-600 text-xs">Nenhum serviço encontrado.</div>
        )}
      </div>
    </div>
  );
