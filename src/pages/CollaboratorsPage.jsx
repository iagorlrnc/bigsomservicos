...
export default function CollaboratorsPage({
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      addToast(
        `Acesso liberado como ${userRole === "admin" ? "Administrador" : "Colaborador"}!`,
        "success",
      )
    } catch (err) {
      console.error(err)
  ...
  const handleReject = async (id) => {
    if (
      window.confirm("Deseja realmente rejeitar e excluir esta solicitação?")
    ) {
      try {
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-neutral-500 text-xs mt-1">
            {approvedCollaborators.length} funcionários cadastrados
          </p>
        </div>
        ...
        <div className="flex gap-2.5">
          ...
          >
            <span>Pendentes</span>
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold flex items-center justify-center ${
                pendingCollaborators.length > 0
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {approvedCollaborators.map((c) => (
          <div
            key={c.id}
            className="bg-[#0d0d0d] border border-neutral-900 rounded-xl p-5 hover:border-neutral-850 transition-colors flex items-center gap-4"
          >
            ...
            <div>
              <div className="text-neutral-200 font-semibold text-sm leading-snug flex items-center gap-2">
                <span>{c.name}</span>
                {c.user_role === "admin" && (
                  <span className="bg-neutral-900 border border-neutral-800 text-neutral-500 text-[8px] font-bold uppercase px-1 rounded">
                    Admin
                  </span>
                )}
              </div>
              <div className="text-[#34d980] text-[10px] uppercase font-semibold tracking-wider mt-0.5">
                {c.role || "Membro da equipe"}
              </div>
              <div className="text-neutral-650 text-xs mt-1">
      ...
      {showForm && (
        <ModalOverlay onClose={() => setShowForm(false)}>
          <form
            ...
            className="bg-[#0d0d0d] border border-neutral-900 rounded-2xl p-5 sm:p-7 w-full max-w-[420px] max-h-[calc(100vh-3rem)] overflow-y-auto animate-scale-up space-y-4"
          >
            ...
            <div className="flex justify-between items-center mb-1">
              <div>
                ...
                <p className="text-neutral-500 text-xs mt-0.5">
                  Defina o papel do usuário e libere o acesso ao painel.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2.5 pt-2 max-h-[50vh] overflow-y-auto pr-1">
              {pendingCollaborators.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="bg-[#070707] border border-neutral-900 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        <div className="text-neutral-250 font-semibold text-xs truncate">
                          {c.name}
                        </div>
                        <div className="text-neutral-600 text-[10px] mt-0.5 truncate">
                          {c.email}
                        </div>
                        {c.phone && (
                          <div className="text-neutral-650 text-[10px] mt-0.5">
                            {c.phone}
                          </div>
                        )}
                      </div>
